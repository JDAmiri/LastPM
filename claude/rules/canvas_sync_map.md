---
description: Phase 4.5 Canvas Sync table. Read immediately after saving any artifact to identify which canvas fields to update.
paths: ["02_Product_Workspace/**/*.md", "01_Global_Context/03_Venture_Strategy/**/*.md"]
---

# Canvas Sync Map

> **Used by Phase 4.5 of CLAUDE.md.**
> After an agent saves its artifact, the orchestrator reads this file, looks up the agent name, and offers to write the relevant fields back to the Canvas Vault. Multiple rows per agent are supported. All matching rows are batched into one bundled offer to the PM.
