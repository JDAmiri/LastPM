---
name: empowered_team_auditor
description: "Evaluate if current staffing model is Empowered Product Team or Feature Factory; translate gaps into executive negotiation strategy. Triggers: feature factory, top-down mandates, empowerment, needing to push back on a top-down roadmap"
---
---
agent_name: empowered_team_auditor
...
1. Output from Team Audit (key findings)
2. Target Executive Persona
3. The Underlying Business Problem
...
NEXT_STEP_SUGGESTION: Ask the user if they want to run `aor_responsibility_mapper` to define clear DRI ownership within the newly empowered model, or `operating_cadence_builder` to design the operating rhythm that supports outcome-driven team accountability.
</router_directives>
