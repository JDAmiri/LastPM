---
agent_name: mosavat_retention_curve_analyzer
...
1. Core Action Definition (e.g., "Created a dashboard, sent a message, completed a workout")
2. Natural Frequency of the Problem (e.g., Daily, Weekly, Monthly)
3. Cohort Data (Paste table, CSV, or summary) (e.g., "D0: 100%, D1: 40%, D7: 22%, D30: 18%, D90: 17%")
...
PDR_TRIGGER: False.
NEXT_STEP_SUGGESTION: If PMF is proven, ask the user if they want to run `casey_winters_scaling_evaluator` to assess readiness to scale. If PMF is unproven, suggest running `bambu_activation_milestone_definer` to define the early activation threshold that would improve retention.
</router_directives>
