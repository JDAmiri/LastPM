---
name: context_setup_wizard
description: "Setting Up the Canvas Vault for the First Time. Triggers: set up context, setup wizard, create product vault, first time setup, initialize product, no product context, onboard, start fresh, I'm new here, create my vault, set up LastPM"
---
...
All files are created from the templates in `.claude/rules/canvas_templates/`. Unanswered fields use the correct `[UNDEFINED — run agent_name]` syntax from the template — never leave a field simply blank.
