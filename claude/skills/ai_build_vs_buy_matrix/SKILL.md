---
name: ai_build_vs_buy_matrix
description: "Evaluate optimal AI model strategy (API vs Fine-tune vs Train from scratch). Routed via scoring when Feasibility score is 0-3 (AI R&D risk). Triggers: build vs buy, custom model, open source, engineering suggests a custom model, unit economics are failing and cost latency scaling is an issue"
---
---
...
SEARCH_QUERIES:
  - "[AI vendor/API] pricing latency SLA [current year]"
  - "[Industry] AI build vs buy cost comparison"
FETCH_TARGETS: []
ENRICHMENT_GOAL: Validate vendor pricing, latency benchmarks, and model capability claims against current published data. Only search if PM did not provide specific vendor details.
</research_directives>
