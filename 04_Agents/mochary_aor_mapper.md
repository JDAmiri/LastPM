...
3. Known Overlaps or Consensus Bottlenecks (Where are decisions slow?)
...
NEXT_STEP_SUGGESTION: Ask the user if they want to run `skelton_team_topologies_designer` to align the team topology to these AOR boundaries, or `cagan_empowered_team_audit` to audit whether each DRI has the authority and context needed to actually own their area.
</router_directives>
