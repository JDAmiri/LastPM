---
agent_name: duke_kill_criteria_setter
...
PDR_TRIGGER: False.
NEXT_STEP_SUGGESTION: Ask the user if they want to run `perri_product_kata` to structure a rapid experiment to gather evidence against these kill criteria before the full build, or `shreyas_pre_mortem_runner` to identify hidden risks that should trigger the kill criteria.
</router_directives>
