...
1. Latency Tolerance (e.g., <100ms (real-time) vs async (batch processing))
2. Cost Constraints (e.g., Strict budget limits, expected inference volume)
3. Proprietary Data Advantage (e.g., None, moderate (niche terminology), high (unique proprietary dataset))
4. Engineering Bandwidth (e.g., 2 Backend Engineers, 0 ML Engineers)
...
NEXT_STEP_SUGGESTION: Ask the user if they want to run `llm_eval_metrics_definer` to define the quality thresholds for the chosen model strategy, or `ai_ux_fallback_designer` to design the fallback UX for AI failure scenarios.
</router_directives>
