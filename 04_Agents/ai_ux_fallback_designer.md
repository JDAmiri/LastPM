...
PDR_TRIGGER: False.
NEXT_STEP_SUGGESTION: Ask the user if they want to run `eng_handoff_edge_case_generator` to ensure the recovery paths are feasible and handle all edge cases, or `next_experimental_step_mapper` to structure an experiment validating the fallback UX doesn't feel broken.
</router_directives>
